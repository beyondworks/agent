...
import uxData from '../data/ux-guidelines.json';
...
const data = uxData as unknown as UxRule[];
...
function groupByCategory(items: UxRule[]): Record<string, UxRule[]> {
  const g: Record<string, UxRule[]> = {};
...
const UxGuidelinesPage = () => {
  const grouped = groupByCategory(data);
  return (
    <div style={{ padding: 32, background: BG_BASE, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ color: TEXT_PRIMARY, fontSize: 28, fontWeight: 800, marginBottom: 8 }}>UX Guidelines</h1>
      <p style={{ color: TEXT_SECONDARY, fontSize: 14, marginBottom: 32 }}>{data.length} rules across {Object.keys(grouped).length} categories</p>
...
const meta: Meta = {
  title: 'Theme System/UX Guidelines',
  parameters: { layout: 'fullscreen' },
};
export default meta;
type Story = StoryObj;
export const Catalog: Story = { render: () => <UxGuidelinesPage /> };
